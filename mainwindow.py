...
btn_get_file_path=tk.Button(frm,text='Choisir un text ：',command=get_file)
...
btn_start_rec=tk.Button(frm,text='Commencer',command=start_rec,width=10)
btn_start_rec.grid(row=2,column=0)

lb_Status = tk.Label(frm, text='Prêt', anchor='w', fg='green')
lb_Status.grid(row=2,column=1)

btn_stop_rec=tk.Button(frm,text="Terminé",command=stop_rec)
